...
func TestSplitRecordByShards_UnsupportedColumnTypeReturnsError(t *testing.T) {
	alloc := memory.NewGoAllocator()
	schema := arrow.NewSchema([]arrow.Field{
		{Name: "unsupported", Type: arrow.PrimitiveTypes.Int32},
	}, nil)

	int32Builder := array.NewInt32Builder(alloc)
	int32Builder.Append(42)

	rec := array.NewRecordBatch(schema, []arrow.Array{int32Builder.NewArray()}, 1)
	defer rec.Release()

	results, err := splitRecordByShards(rec, []int{0}, 2)
	require.Error(t, err)
