...
func (r *scanTimeRangePushup) applyToTargets(node Node, timeRange TimeRange) bool {
	...
	case *RangeAggregation:
		if node.Step > 0 { // only apply optimization to range queries
			trSteppedStart := time.UnixMilli((timeRange.Start.UnixNano() / node.Step.Nanoseconds()) * node.Step.Nanoseconds() / 1_000_000).UTC()

			endPlusRange := timeRange.End.Add(node.Range)
			trSteppedEnd := time.UnixMilli((endPlusRange.UnixNano() / node.Step.Nanoseconds()) * node.Step.Nanoseconds() / 1_000_000).UTC()
